...
# Diagrama: Arquitectura del sistema (arqL)

> Diagrama que representa la arquitectura descrita en el texto: cliente móvil multiplataforma (KMM) con capas Clean Architecture / Hexagonal, y backend Node.js/Express con persistencia en MariaDB. El API REST es stateless y usa JWT con refresh.

```mermaid
flowchart LR
    subgraph Cliente_Móvil[Aplicación móvil (Cliente)]
        direction TB
        KMM["Lógica compartida\n(KMM)"]
        subgraph UI[\nInterfaces nativas (Presentación)\nMVVM]
            direction LR
            Android["Android\nJetpack Compose\n(Views)"]
            iOS["iOS\nSwiftUI\n(Views)"]
        ...
        subgraph Dominio[Dominio]
            Entities["Entidades & Casos de uso\n(Use Cases)"]
            ReposInterfaces["Interfaces de repositorio\n(Ports)"]
        end
        subgraph Datos[Datos / Infraestructura]
            Ktor["Cliente HTTP: Ktor\nDTOs, Repositorios (Adapters)"]
            Storage["Almacenamiento local\n(DataStore) (opcional)"]
        end
        Android --- KMM
        iOS --- KMM
        KMM --> Entities
        Entities --> ReposInterfaces
        ReposInterfaces --> Ktor
        ReposInterfaces --> Storage
    end

    subgraph Red[Red / API REST]
        HTTPS[["HTTPS\n(stateless REST)"]]
    end

    subgraph Backend[Servidor (Backend) - Node.js / Express]
        direction TB
        Routing["Routing\n(endpoints HTTP)"]
        Controllers["Controladores / Servicios\n(Lógica de negocio)\nUse Cases / Services"]
        Persistence["Persistencia / Repositorios\nMariaDB (SQL)\nAcceso a BD"]
    end

    %% Conexiones principales
    Ktor -- "HTTPS (REST) + JWT" --> HTTPS
    HTTPS --> Routing
    Routing --> Controllers
    Controllers --> Persistence

    %% JWT / Refresh flow
    subgraph AuthLegend[Autenticación JWT]
        A1["Auth: /login -> accessToken + refreshToken"]
        A2["Acceso: Authorization: Bearer <accessToken>"]
        A3["Refresco: /mobile/refresh (refreshToken) -> nuevo accessToken"]
    end
    Controllers --- A1
    Ktor --- A2
    Ktor --- A3

    %% Notas y separación de capas
    classDef cliente fill:#f9f,stroke:#333,stroke-width:1px;
    classDef servidor fill:#ff9,stroke:#333,stroke-width:1px;
    class Cliente_Móvil cliente;
    class Backend servidor;

    %% Hexagonal / adapters hint
    ReposInterfaces -.->|"Ports / Adapters"| Ktor
    ReposInterfaces -.->|"Ports / Adapters"| Storage

    %% Stateless label
    noteOver HTTPS,Controllers "API stateless: cada petición contiene credenciales/tokens"
```
...
* **KMM**: lógica compartida entre Android e iOS (use cases, modelos de dominio).
* **Jetpack Compose / SwiftUI**: interfaces declarativas nativas. Los *ViewModels* gestionan el estado y llaman a casos de uso.
* **Ktor**: cliente HTTP en la capa de datos que llama a la API REST.
* **Node.js + Express**: servidor que expone endpoints, maneja autenticación (JWT), validaciones y lógica de negocio.
* **MariaDB**: base de datos relacional usada por la capa de persistencia en el backend.
* **JWT + refresh**: esquema donde la app guarda accessToken + refreshToken; el primero en cada petición, el segundo para renovar cuando expire.

---

## Sugerencias de uso y exportación

* Puedes exportar este diagrama (Mermaid) a SVG/PNG con cualquier editor compatible (e.g. VSCode + extensión "Markdown Preview Mermaid") o con herramientas online.
* Si quieres, lo puedo convertir a una imagen (SVG/PNG) aquí mismo y ajustar el diseño: indícame tamaño, orientación (horizontal/vertical) y si prefieres colores corporativos.

*Diagrama generado según la descripción proporcionada, puedes editar o pedirme una versión más técnica (por ejemplo, añadiendo puertos/DTOs concretos o rutas de ejemplo).*
